...
`v0.3.2` 是适用于 CLIProxyAPI v7（CPA）的原生视觉桥接插件。它让不支持图片的长上下文文本模型继续担任主模型：视觉模型只负责把图片转换成受控文本，最终理解、推理和回答仍由 GLM 等首选文本模型完成。
...
Agent、Claude Code、Codex 或其他 OpenAI 兼容客户端只需把 `model` 设置为该名称。客户端不需要知道内部的文本模型和视觉模型名称。
...
`vision_timeout_seconds` 在当前 CPA Host ABI 中不是硬取消。CPA 的非流式 Host 调用开始后，插件无法在首包前中止上游连接。因此采用安全的软预算：
...
dist/glm-vision-combo_0.3.2_linux_amd64.zip
dist/checksums.txt
```
...
5. 重启 CPA 或重新加载插件。
6. 在日志中确认 `GLM Vision Bridge version=0.3.2`。
