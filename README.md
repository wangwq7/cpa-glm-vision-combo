...
`v0.4.7` 是适用于 CLIProxyAPI v7（CPA）的原生视觉桥接插件。它让不支持图片的长上下文文本模型继续担任主模型：视觉模型只负责把图片转换成受控文本，最终理解、推理和回答仍由 GLM 等首选文本模型完成。
...
- 切换文本模型前，较早且完整配对的工具调用/结果会先做本地确定性归档；保留全部系统/开发者规则、用户文本、普通回答及最近工具状态，不调用额外模型，也不留下孤立工具消息。
- 长会话首次达到阈值时建立可持久复用的历史摘要检查点；后续追加少量对话直接复用，只有新增尾部再次逼近预算时才增量更新一次。
...
dist/glm-vision-combo_0.4.7_linux_amd64.zip
dist/checksums.txt
```
...
5. 重启 CPA 或重新加载插件。
6. 在日志中确认 `GLM Vision Bridge version=0.4.7`。
