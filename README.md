...
`v0.5` 是适用于 CLIProxyAPI v7（CPA）的原生视觉桥接插件。它让不支持图片的长上下文文本模型继续担任主模型：视觉模型只负责把图片转换成受控文本，最终理解、推理和回答仍由 GLM 等首选文本模型完成。
...
- 非活动工具续轮中，较早且完整配对的工具调用/结果才会做本地确定性归档；保护窗口按真实用户轮次计算，不按 Responses 的 reasoning/function_call 项目计数。客户端正在提交工具结果时，完整工具状态原样保留，不调用额外模型，也不留下孤立工具消息。
- 长会话首次达到阈值时建立可持久复用的历史摘要检查点；摘要边界同样按真实用户轮次选择，绝不跨入当前活动工具链。若当前任务没有可安全摘要的旧前缀且仍在工作预算内，则原样透传；超过预算则明确报错，不静默删除工具状态。
...
5. 重启 CPA 或重新加载插件。
6. 在日志中确认 `GLM Vision Bridge version=0.5`。
...
- 模型切换后的第一轮会丢失原供应商的前缀缓存；插件会先归档旧工具执行轨迹，再判断是否需要摘要检查点，以减少冷传给新模型的上下文。
...
- 新增跨 OpenAI Chat、OpenAI Responses 和 Claude Messages 的旧工具轨迹本地归档：仅在非活动工具续轮中移除保护窗口之外且完整配对的调用/结果；保护窗口按真实用户轮次计算，混合消息中的普通文本继续保留。
- 归档只在至少节省 4 KiB 时采用；无工具历史走快速扫描并原样返回，小轨迹、未配对轨迹和近期工具状态不会被修改。
- 自动历史摘要检查点改为按真实用户轮次选边界，并把当前活动工具链作为不可跨越的硬边界；单个长任务低于工作预算时不再被强制摘要。
