...
`v0.4.1` 是适用于 CLIProxyAPI v7（CPA）的原生视觉桥接插件。它让不支持图片的长上下文文本模型继续担任主模型：视觉模型只负责把图片转换成受控文本，最终理解、推理和回答仍由 GLM 等首选文本模型完成。
...
dist/glm-vision-combo_0.4.1_linux_amd64.zip
dist/checksums.txt
```
...
4. 重启 CPA 或重新加载插件。
5. 在日志中确认 `GLM Vision Bridge version=0.4.1`。
