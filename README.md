...
Agent、Claude Code、Codex 或其他 OpenAI/Claude 兼容客户端只需把 `model` 设置为其中一个名称。插件会保持客户端协议：OpenAI 请求得到 OpenAI 响应，Claude `/v1/messages` 请求得到 Claude 响应；客户端不需要知道内部的文本模型和视觉模型名称。
...
4. 重启 CPA 或重新加载插件。
5. 在日志中确认 `GLM Vision Bridge version=0.4.2`。
