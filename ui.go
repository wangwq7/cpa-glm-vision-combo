...
func managementHTML(cfg runtimeConfig) (page string) {
	defer func() {
		page = strings.NewReplacer(
			...
			"视觉模型只负责把图片转成不可信的事实文本；主任务仍由首选文本模型完成。历史图片按需恢复，长对话超过阈值后自动压缩。", "自动适配 OpenAI Chat、Responses 与 Claude Messages。旧图按需恢复；工具调用与结果完整透传，长对话达到阈值后再复用摘要检查点。",
			...
			"按需恢复减少非图片续问中的上下文；自动压缩仅在接近阈值时触发。", "未引用旧图不再逐张解码；工具调用与结果完整保留；摘要检查点仅在仍接近预算时创建或更新。",
			...
			"这些最近消息保持原文，不进入摘要。", "这些最近消息及其工具状态保持原文；工具调用与结果不做独立删除。",
