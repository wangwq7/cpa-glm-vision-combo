...
func managementHTML(cfg runtimeConfig) (page string) {
	defer func() {
		page = strings.NewReplacer(
			...
			"视觉模型只负责把图片转成不可信的事实文本；主任务仍由首选文本模型完成。历史图片按需恢复，长对话超过阈值后自动压缩。", "自动适配 OpenAI Chat、Responses 与 Claude Messages。旧图按需恢复；较早且完整配对的工具轨迹先在本地归档，长对话再复用摘要检查点。",
			"当前图片完整识别；旧图片默认只保留短摘要，明确引用时恢复详情。", "当前图片完整识别；未引用旧图合并为固定归档标记，明确引用时按上限恢复。",
			"<strong>纯文本轮：</strong>完全跳过视觉链，只有达到压缩阈值时才多一次摘要调用。", "<strong>纯文本轮：</strong>先归档旧工具执行轨迹；仍超阈值时建立或复用摘要检查点。",
			"按需恢复减少非图片续问中的上下文；自动压缩仅在接近阈值时触发。", "未引用旧图不再逐张解码；旧工具轨迹先本地归档；摘要检查点仅在仍接近预算时创建或更新。",
			...
			"这些最近消息保持原文，不进入摘要。", "这些最近消息及其工具状态保持原文；更早且完整配对的工具轨迹可先本地归档。",
			...
			"记录将展示缓存命中、视觉回退、自动压缩、文本回退和完整耗时。", "记录将展示历史图片处理、旧工具轨迹归档、摘要检查点、模型回退和完整耗时。",
		).Replace(page)
	}()
