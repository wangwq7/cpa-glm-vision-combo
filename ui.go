...
func managementHTML(cfg runtimeConfig) (page string) {
	defer func() {
		page = strings.NewReplacer(
			...
			"视觉模型只负责把图片转成不可信的事实文本；主任务仍由首选文本模型完成。历史图片按需恢复，长对话超过阈值后自动压缩。", "自动适配 OpenAI Chat 与 Claude Messages 协议。视觉模型只负责图片转写；旧图使用固定归档标记，长对话复用增量摘要检查点。",
			"当前图片完整识别；旧图片默认只保留短摘要，明确引用时恢复详情。", "当前图片完整识别；未引用旧图合并为固定归档标记，明确引用时按上限恢复。",
			"<strong>纯文本轮：</strong>完全跳过视觉链，只有达到压缩阈值时才多一次摘要调用。", "<strong>纯文本轮：</strong>完全跳过视觉链；首次超阈值建立检查点，后续追加少量对话直接复用。",
			"按需恢复减少非图片续问中的上下文；自动压缩仅在接近阈值时触发。", "未引用旧图不再逐张解码；摘要检查点只在首次超阈值或新增尾部再次逼近预算时更新。",
